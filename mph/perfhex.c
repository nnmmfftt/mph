 ...
 **/
  ...
  if (x != y && x != z && y != z)
  {
    ...
    {
      /* h3b: 0,3,2 */
      sprintf(final->line[0], "  uint32_t rsl = ((val & 3) ^ %ld);\n",
	      find_adder(x,y,z));
    }
    return;
  }
 ...
 */
static void hexn(key *keys, uint32_t salt, uint32_t alen, uint32_t blen, uint32_t final){
/** 
 * key     *keys;
 * uint32_t      salt;
 * uint32_t      alen;
 ...
 */
int inithex(key *keys, uint32_t nkeys, uint32_t alen, uint32_t blen,
            uint32_t smax, uint32_t salt, gencode *final, hashform *form){
/**
